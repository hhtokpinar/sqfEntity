...
import 'package:points_of_sale/data_base/DBhelper.dart';
...
class Auth with ChangeNotifier {
  ...
  static List<String> pinCodeProfilekeys = <String>['phone', 'password'];
  get userPicture => null;
  Future<bool> login(String email, String pass, BuildContext context) async {
    // bool login = await databaseHelper.readUser(email, pass);
    print("login  $login");
    // return login;
  }

  Future<void> register(BuildContext context, String username, String pass,
      String email, String mobile, String dob, String cat) async {
    print(
        "user is trying to register here $username  $pass  $email  $mobile $dob");
    await location.getLocation().then((LocationData value) async {
      // await databaseHelper.adduser(User(email, username, mobile, pass, dob,
      //     value.latitude, value.longitude, cat));
    });
  ...
  updateProfile(String username, String brithdate, String email) {}
