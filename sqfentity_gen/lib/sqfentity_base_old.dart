...
class SqfEntityBuilder {
  const SqfEntityBuilder(this.model,
      {this.formControllers, this.keepFieldNamesAsOriginal});
  final SqfEntityModel model;
  final List<SqfEntityTable> formControllers;
  final bool keepFieldNamesAsOriginal;
}

class SqfEntityBuilderForm {
  const SqfEntityBuilderForm(this.table,
      {this.formListTitleField, this.formListSubTitleField, this.hasSubItems});
...
class SqfEntityModel {
  ...
  final dynamic openDatabaseOptions;
...
''';
  ...
  String __createObjectField() {
    ...
    if (_table.primaryKeyName.isNotEmpty && !_table.primaryKeyName.startsWith('_')) {
      retVal
          .writeln('''static TableField _f${toCamelCase(_table.primaryKeyName)};
  static TableField get ${_table.primaryKeyName} {
  return _f${toCamelCase(_table.primaryKeyName)} = _f${toCamelCase(_table.primaryKeyName)} ?? SqlSyntax.setField(_f${toCamelCase(_table.primaryKeyName)}, '${_table.primaryKeyName.toLowerCase()}', ${DbType.integer.toString()});
  }
''');
    }
    for (SqfEntityFieldType field
        in _table.fields.where((f) => !f.fieldName.startsWith('_'))) {
      if (field is SqfEntityFieldVirtualBase) continue;
      retVal.writeln(
...
class SqfEntityObjectBuilder {
  ...
  String get _createConstructure => __createConstructure(false);
  String get _createBaseConstructure => __createBaseConstructure();
  String get _createListParameterForQuery => __createConstructure(true);
  ...
  String get _hiddenMethod => _table.isOneToOneTable ? '_' : '';
  ...
  class ${_table.modelName} {
    ${_table.modelName}({${_table.primaryKeyName.startsWith('_') || _table.primaryKeyName.isEmpty ? '' : 'this.${_table.primaryKeyName},'} $_createBaseConstructure}) { _setDefaultValues();}
    ${_table.modelName}.withFields($_createConstructure){ _setDefaultValues();}
    ${_table.modelName}.withId($_getByIdParameters, $_createConstructure){ _setDefaultValues();}
    ...
    Future<Map<String, dynamic>> toMapWithChilds([bool forQuery = false, bool forJson=false]) async {
      final map = <String, dynamic>{};
      $_toMapString
    ...
    List<dynamic> toArgs() {
      return[$_getByIdParameters,${_createListParameterForQuery.replaceAll('this.', '')}];   
    }  
    ...
    */
    ...
    static Future<List<${_table.modelName}>> fromMapList(List<dynamic> data,{bool preload=false, List<String> preloadFields, bool loadParents=false}) async{
    ...
    Future<${_table.modelName}> getById($_getByIdParametersWithTypes, {bool preload=false, List<String> preloadFields,bool loadParents=false}) async{
    ...
    /// <returns>Returns ${_table.primaryKeyName}
    Future<int> _upsert() async {
       try {
         if (await _mn${_table.modelName}.rawInsert(
          'INSERT OR REPLACE INTO ${_table.tableName} (${_table.primaryKeyName}, ${_createConstructure.replaceAll('this.', '')})  VALUES (?,$_createConstructureArgs)', [${_table.primaryKeyName},${_createListParameterForQuery.replaceAll('this.', '')}]) ==
           1) {
          saveResult = BoolResult(success: true, successMessage: '${_table.modelName} ${_table.primaryKeyName}=\$${_table.primaryKeyName} updated successfully');
          } else {
        saveResult = BoolResult(
            success: false, errorMessage: '${_table.modelName} ${_table.primaryKeyName}=\$${_table.primaryKeyName} did not update');
         }
          return ${_table.primaryKeyType == PrimaryKeyType.text ? '1' : '${_table.primaryKeyName}'};
       } catch (e) {
  ...
  String __createProperties() {
    final _retVal = StringBuffer(
        '${_table.primaryKeyType == PrimaryKeyType.text ? 'String' : 'int'} ${_table.primaryKeyName};');
    ...
    _retVal
      ..writeln(_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental
          ? 'bool isSaved;'
          : '')
  ...
  String __createConstructure(bool isQuery) {
    final _retVal = StringBuffer();
    for (final field in _table.fields) {
      if (field is SqfEntityFieldVirtualBase) continue;
  ...
  String __toFromMapString() {
    ...
    if (_table.primaryKeyType == PrimaryKeyType.text) {
      _retVal.writeln(
          '${_table.primaryKeyName} = o[\'${_table.primaryKeyName}\'] as String;');
    } else {
      _retVal.writeln(
          '${_table.primaryKeyName} = int.tryParse(o[\'${_table.primaryKeyName}\'].toString());');
    }
    for (final field in _table.fields) {
  ...
  String __createObjectRelations() {
    ...
    for (final field
      ...
      if (field is SqfEntityFieldRelationshipBase &&
        ...
        Future<$modelName> get$funcName({bool loadParents=false}) async{
            final _obj = await $modelName().getById(${field.relationshipFields.join(',')}, loadParents: loadParents);
            return _obj;
          }
        ''';
  ...
  String __createObjectCollections() {
    ...
    for (var tableCollection in _table.collections) {
      ...
      if (tableCollection.childTableField.relationType ==
        ...
        retVal.writeln(
            '${tableCollection.childTable.modelName} ${tableCollection.childTable.modelName.toLowerCase()};');
        continue;
      }
      final childTableFieldTablePrimaryKeyName =
          tableCollection.childTableField.table == null
              ? _table.primaryKeyName
              : tableCollection.childTableField.table.primaryKeyName;
      if (_table.tableName == tableCollection.childTable.tableName) {
        retVal.writeln('''
            /// to load children of items to this field, use preload parameter. Ex: toList(preload:true) or toSingle(preload:true) or getById(preload:true)
            /// You can also specify this object into certain preload fields. Ex: toList(preload:true, preloadFields:['pl$funcName', 'plField2'..]) or so on..
            List<${_table.modelName}> pl$funcName;
            /// get ${_table.modelName}(s) filtered by ${_table.primaryKeyName}=${tableCollection.childTableField.fieldName}
              ${_table.modelName}FilterBuilder get$funcName({List<String> columnsToSelect, bool getIsDeleted}){
           return ${_table.modelName}().select(columnsToSelect: columnsToSelect,getIsDeleted: getIsDeleted).${_table.primaryKeyName}.equals(${tableCollection.childTableField.fieldName}).and;
          }''');
      } else {
        retVal.writeln(
            '''/// to load children of items to this field, use preload parameter. Ex: toList(preload:true) or toSingle(preload:true) or getById(preload:true)
    ...
    return retVal.length > 0
        ? '\n// COLLECTIONS & VIRTUALS (${_table.modelName})\n${retVal.toString()}// END COLLECTIONS & VIRTUALS (${_table.modelName})\n'
        : '';
  }
  ...
  String __createObjectCollectionsFromMap() {
    final retVal = StringBuffer();
    final collections = <String>[];
    ...
    for (var tableCollection in _table.collections) {
      ...
      if (tableCollection.childTableField.table != null) {
        final relationName = tableCollection.childTable.modelName;
        if (tableCollection.childTableField.relationType ==
            RelationType.ONE_TO_ONE) {
          //retVal.writeln('''if (!forQuery && $relationName != null) {map['$relationName'] = $relationName.toMap();}''');
        } else {
          //retVal.writeln('''if (!forQuery) {map['$funcName'] = await get$funcName().toMapList();}''');
        }
        retVal.writeln(
            'pl$funcName = o[\'$funcName\'] != null ? $relationName.fromMap(o[\'$funcName\'] as Map<String, dynamic>) : null;');
      }
    }
    return retVal.length > 0
        ? '\n// COLLECTIONS FromMAP (${_table.modelName})\n${retVal.toString()}// END COLLECTIONS FromMAP (${_table.modelName})\n'
        : '';
  }
  ...
  String __deleteMethodSingle() {
    ...
    return retVal += '''
      ...
      return _mn${_table.modelName}.delete(QueryParams(whereString: '${_table.primaryKeyName}=?', whereArguments: [${_table.primaryKeyName}]));}
      else {
      return _mn${_table.modelName}.updateBatch(QueryParams(whereString: '${_table.primaryKeyName}=?', whereArguments: [${_table.primaryKeyName}]), {'isDeleted': 1});}''';
  }
  ...
  String __recoverMethodSingle() {
    ...
    retVal += '''
      {return _mn${_table.modelName}.updateBatch(QueryParams(whereString: '${_table.primaryKeyName}=?', whereArguments: [${_table.primaryKeyName}]), {'isDeleted': 0});}''';
  ...
  String __saveAllMethod() {
    ...
    Future<List<BoolResult>> saveAll(List<${_table.modelName}> ${toPluralName(_table._modelLowerCase)}) async {
      final results = _mn${_table.modelName}.saveAll('INSERT OR REPLACE INTO ${_table.tableName} (${_table.primaryKeyName}, ${_createConstructure.replaceAll('this.', '')})  VALUES (?,$_createConstructureArgs)',${toPluralName(_table._modelLowerCase)});
      return results;
    }
  ''';
  }
  ...
  String __upsertAllMethod() {
    ...
    /// 
    /// Returns a <List<BoolResult>> 
    Future<List<BoolResult>> ${_hiddenMethod}upsertAll(List<${_table.modelName}> ${toPluralName(_table._modelLowerCase)}) async {
      final results = await _mn${_table.modelName}.rawInsertAll(
          'INSERT OR REPLACE INTO ${_table.tableName} (${_table.primaryKeyName}, ${_createConstructure.replaceAll('this.', '')})  VALUES (?,$_createConstructureArgs)', ${toPluralName(_table._modelLowerCase)});
      return results;
    }''';
  }
  ...
  String __saveMethod() {
    ...
    for (var sequencedField
      ...
      seq.writeln(
          'if (${_table.primaryKeyName} != null) { ${sequencedField.fieldName} = await ${sequencedField.sequencedBy.modelName}().nextVal(); save();}');
    }
    final retVal = StringBuffer();
    if (_table.primaryKeyType == PrimaryKeyType.text) {
      retVal.write('''
  
    /// Saves the (${_table.modelName}) object. If the Primary Key (${_table.primaryKeyName}) field is null, returns Error. 
    /// 
    /// INSERTS (If not exist) OR REPLACES (If exist) data while Primary Key is not null. 
    /// 
    /// Call the saveAs() method if you do not want to save it when there is another row with the same ${_table.primaryKeyName}
    ...
    Future<BoolResult> ${_hiddenMethod}save() async {
      ...
      try {         
       ...
       'INSERT ${_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental ? '\${isSaved ? \'OR REPLACE\':\'\'}' : 'OR REPLACE'} INTO ${_table.tableName} (${_table.primaryKeyName}, ${_createConstructure.replaceAll("this.", "")})  VALUES (?,$_createConstructureArgs)', toArgs());
        result.success=true;
        ${_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental ? 'isSaved = true;' : ''}
  ...
  ''');
    ...
    /// Use this method if you do not want to update existing row when conflicts another row that have the same ${_table.primaryKeyName}
    ...
    Future<BoolResult> ${_hiddenMethod}saveAs() async {
      ...
      try {         
        await _mn${_table.modelName}.rawInsert(
          'INSERT INTO ${_table.tableName} (${_table.primaryKeyName}, ${_createConstructure.replaceAll("this.", "")})  VALUES (?,$_createConstructureArgs)', [${_table.primaryKeyName},${_createConstructure.replaceAll("this.", "")}]);
      ${seq.toString()}              
        result.success=true;
        ${_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental ? 'isSaved = true;' : ''}
      } catch (e){
    ...
    /// Saves the (${_table.modelName}) object. If the ${_table.primaryKeyName} field is null, saves as a new record and returns new ${_table.primaryKeyName}, if ${_table.primaryKeyName} is not null then updates record
    
    /// <returns>Returns ${_table.primaryKeyName}
    Future<int> ${_hiddenMethod}save() async {
      if (${_table.primaryKeyName} == null || ${_table.primaryKeyName} == 0 ${_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental ? '|| !isSaved' : ''}) {
        ${_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental ? '' : '${_table.primaryKeyName} ='} await _mn${_table.modelName}.insert(this);
        ${_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental ? 'if (saveResult.success) isSaved = true;' : ''}
        ${seq.toString()}
          }
      else {
    ...
    Future<int> ${_hiddenMethod}saveAs() async {
      ${_table.primaryKeyType != PrimaryKeyType.integer_auto_incremental ? 'isSaved = false;' : ''}
      ${_table.primaryKeyType == PrimaryKeyType.integer_auto_incremental ? '${_table.primaryKeyName} = null;' : ''}
      $_toOnetoOneSaveAsCode
...
String __toOnetoOneCollections(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections) {
    ...
    if (tableCollection.childTableField.relationType ==
      ...
      */
      retVal.writeln(
          '''obj.${tableCollection.childTable.modelName.toLowerCase()} = await ${tableCollection.childTable.modelName}()
          .select()
...
String __toOnetoManyCollections(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections) {
    ...
    retVal.writeln(
        'if (preloadFields == null || preloadFields.contains(\'pl$funcName\')) {obj.pl$funcName = obj.pl$funcName ?? await obj.get$funcName().toList(preload: preload, preloadFields: preloadFields, loadParents: false);}');
    collections.add(funcName);
  }
...
String __toOnetoOneSaveCode(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections) {
    if (tableCollection.childTableField.relationType ==
      ...
      retVal.writeln(
          '''$objName?.${tableCollection.childTableField.fieldName} = $childTableFieldTablePrimaryKeyName;
                        await $objName?._save();''');
    }
  }
  return retVal.length > 0
...
String __toOnetoOneSaveAsCode(SqfEntityTableBase _table) {
  ...
  for (var tableCollection in _table.collections) {
    if (tableCollection.childTableField.relationType ==
      ...
      final objName = tableCollection.childTable.modelName.toLowerCase();
      retVal.writeln(
          '$objName.${tableCollection.childTable.primaryKeyName} = null;');
    }
  }
  return retVal.length > 0
...
class ${_table.modelName}Manager extends SqfEntityProvider {
  ...
  static List<String> _primaryKeyList = ['${_table.primaryKeyNames != null ? _table.primaryKeyNames.join('\',\'') : _table.primaryKeyName}'];
  static String _whereStr = '${_table.primaryKeyNames != null ? _table.primaryKeyNames.join('=?, AND') : _table.primaryKeyName}=?';
}
...
Future<BoolResult> delete([bool hardDelete=false]) async {
  ...
  Future<BoolResult> update(Map<String, dynamic> values) {
    ...
    if (qparams.limit > 0 || qparams.offset > 0) {
      qparams.whereString = '${_table.primaryKeyName} IN (SELECT ${_table.primaryKeyName} from ${_table.tableName} \${qparams.whereString.isNotEmpty ? 'WHERE \${qparams.whereString}': ''}\${qparams.limit>0 ? ' LIMIT \${qparams.limit}':''}\${qparams.offset>0 ? ' OFFSET \${qparams.offset}':''})';
    }
  ...
  Future<${_table.modelName}> toSingle({bool preload=false, List<String> preloadFields, bool loadParents=false}) async{
    _pagesize = 1;
    _buildParameters();
  ...
  /// 
  /// <returns>List<${_table.modelName}>
  Future<List<${_table.modelName}>> toList({bool preload=false, List<String> preloadFields, bool loadParents=false}) async {
    final data = await toMapList();
    final List<${_table.modelName}> ${toPluralLowerName(_table._modelLowerCase)}Data = await ${_table.modelName}.fromMapList(data,preload: preload, preloadFields: preloadFields, loadParents: loadParents);
  ...
  Future<List<DropdownMenuItem<int>>> toDropDownMenuInt(
      String displayTextColumn,
	  [VoidCallback Function(List<DropdownMenuItem<int>> o) dropDownMenu]) async {
    _buildParameters();
    qparams.selectColumns=['${_table.primaryKeyName}',displayTextColumn];
    final ${toPluralLowerName(_table._modelLowerCase)}Future = _obj._mn${_table.modelName}.toList(qparams);
    ...
    for (int i = 0; i < count; i++) {
      items.add(
        DropdownMenuItem(
          value: data[i]['${_table.primaryKeyName}'] as int,
          child: Text(data[i][displayTextColumn].toString()),
        ),
      );
    }
...
Future<List<int>> toListPrimaryKey([bool buildParameters=true]) async {
  ...
  final List<int> ${_table.primaryKeyName}Data = <int>[];
  ...
  final ${_table.primaryKeyName}Future = await _obj._mn${_table.modelName}.toList(qparams);
    ...
    for (int i = 0; i < count; i++) {
      ${_table.primaryKeyName}Data.add(${_table.primaryKeyName}Future[i]['${_table.primaryKeyName}'] as int);
    }
...
Future<List<dynamic>> toListObject([VoidCallback Function(List<dynamic> o) listObject]) async {
  _buildParameters();
...
// endregion ${_table.modelName}FilterBuilder
  ...
  String __createObjectFieldProperty() {
    final retVal = StringBuffer();
    if (!_table.primaryKeyName.startsWith('_')) {
      retVal.writeln('''${_table.modelName}Field _${_table.primaryKeyName};
${_table.modelName}Field get ${_table.primaryKeyName} {
return _${_table.primaryKeyName} = setField(_${_table.primaryKeyName}, '${_table.primaryKeyName}', ${DbType.integer.toString()});
}''');
    }
...
class SqfEntityTableBase {
  ...
  bool keepFieldNamesAsOriginal;
  bool hasSubItems;
  bool isOneToOneTable = false;
  void init() {
    ...
    for (final field in fields) {
      ...
      if (field is SqfEntityFieldRelationshipBase) {
        ...
        if (field.relationType == RelationType.ONE_TO_ONE) {
          isOneToOneTable = true;
          primaryKeyName = '_$primaryKeyName';
          field.fieldName = '_${field.fieldName}';
        }
        field.deleteRule = field.deleteRule ?? DeleteRule.NO_ACTION;
        if (field.isPrimaryKeyField != null && field.isPrimaryKeyField) {
          primaryKeyNames.add(field.fieldName.toLowerCase());
    ...
    //primaryKeyName = keepFieldNamesAsOriginal ? primaryKeyName : tocamelCase(primaryKeyName);
    print(
        '>>>>>>>>>>>>>>>>>>>>>>>>>>>> SqfEntityTableBase of [$tableName]($primaryKeyName) init() successfully');
  }
    ...
    for (SqfEntityFieldType field in fields) {
      if (field is SqfEntityFieldRelationshipBase) {
        ...
        _createTableSQL.write(
            ', FOREIGN KEY(${field.fieldName}) REFERENCES ${field.table.tableName}(${field.table.primaryKeyName}) ON DELETE ${field.deleteRule.toString().replaceAll('_', ' ').replaceAll('DeleteRule.', '')}');
      }
    }
  ...
  void init() {
    if (table != null) {
     ...
     // print('0- relationshipFields added $fieldName on ${table.tableName}');
      ...
      if(fieldName != null && fieldName.isNotEmpty) relationshipFields = <String>[]..add(fieldName);
      print(
...
abstract class SqfEntityModelBase {
  ...
  List<SqfEntityTableBase> controllerTables;
  ...
  void init() {
    for (final table in databaseTables) {
      ...
      //table.dbModel = table.dbModel == null ? instanceName != null ? toCamelCase(instanceName) : toCamelCase(table.tableName): table.dbModel;
    }
  }
}
...
DbType parseDbType(String val) {
  ...
  if (val.contains('char') || val.contains('clob')) {
    ...
    val = 'integer';
  } else if (val.contains('double') || val.contains('float')) {
    val = 'real';
