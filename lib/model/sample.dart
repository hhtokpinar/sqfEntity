...
SqfEntityFieldBase('isActive', DbType.bool, defaultValue: 'true'),
SqfEntityFieldRelationshipBase(TableCategory.getInstance, DeleteRule.CASCADE, defaultValue: '0'),
...
TableTodo.getInstance,
    ...
    sequences = [SequenceIdentitySequence.getInstance,
];
    bundledDatabasePath =
...
class Category {
  ...
  Category.fromMap(Map<String, dynamic> o) {
    id = o['id'] as int;    name = o['name'] as String;
...
ProductFilterBuilder getProducts({List<String> columnsToSelect, bool getIsDeleted}){
         return Product().select(columnsToSelect: columnsToSelect,getIsDeleted: getIsDeleted).categoryId.equals(id).and;
        }
// END COLLECTIONS
...
// COLLECTIONS
if(!forQuery) {map['products'] = await getProducts().toMapList();}
// END COLLECTIONS
  ...
  static Future<List<Category>> fromWebUrl(String url) async {
    try {
      ...
      print('SQFENTITY ERROR Category.fromWebUrl: ErrorMessage: ${e.toString()}');
      return null;
    }
  }
  ...
  Future<BoolResult> delete([bool hardDelete=false]) async {
    ...
    print('SQFENTITIY: recover Category invoked (id=$id)');
        var result= BoolResult();    if(recoverChilds) {result = await Product().select(getIsDeleted: true).isDeleted.equals(true).and.categoryId.equals(id).update({'isDeleted': 0});}
...
// RELATIONSHIPS
  ...
  static Future<List<Product>> fromWebUrl(String url) async {
    try {
      ...
      print('SQFENTITY ERROR Product.fromWebUrl: ErrorMessage: ${e.toString()}');
      return null;
    }
  }
  ...
  Future<int> save() async {
    if (id == null || id == 0) {
      ...
      if (id != null) { rownum = await IdentitySequence().nextVal(); save();}
  ...
  Future<BoolResult> delete([bool hardDelete=false]) async {
    ...
    print('SQFENTITIY: recover Product invoked (id=$id)');
            {return _mnProduct.updateBatch(QueryParams(whereString: 'id=$id'), {'isDeleted': 0});} 
    }
...
static TableField _fDescription;
  static TableField get description {
   return _fDescription = _fDescription ?? SqlSyntax.setField(_fDescription, 'description', DbType.text);
  }

static TableField _fPrice;
  static TableField get price {
   return _fPrice = _fPrice ?? SqlSyntax.setField(_fPrice, 'price', DbType.real);
  }
...
static TableField _fCategoryId;
  static TableField get categoryId {
   return _fCategoryId = _fCategoryId ?? SqlSyntax.setField(_fCategoryId, 'categoryId', DbType.integer);
  }
...
static TableField _fImageUrl;
  static TableField get imageUrl {
   return _fImageUrl = _fImageUrl ?? SqlSyntax.setField(_fImageUrl, 'imageUrl', DbType.text);
  }
...
class IdentitySequence {
  ...
  Future<int> nextVal([VoidCallback nextval(int o)]) async {
    final val = await SequenceManager().sequence(SequenceIdentitySequence.getInstance, true);
    if (nextval != null) {
      nextval(val);
    }
    return val;
  ...
  Future<int> currentVal([VoidCallback currentval(int o)]) async {
    final val = await SequenceManager().sequence(SequenceIdentitySequence.getInstance, false);
    if (currentval != null) {
      currentval(val);
    }
    return val;
  ...
  Future<int> reset([VoidCallback currentval(int o)]) async {
    final val = await SequenceManager().sequence(SequenceIdentitySequence.getInstance, false, reset: true);
    if (currentval != null) {
      currentval(val);
    }
    return val;
