...
class PlaylistTrackAdd extends StatefulWidget {
  PlaylistTrackAdd(this._playlisttrack);
  final dynamic _playlisttrack;
  @override
  State<StatefulWidget> createState() =>
      PlaylistTrackAddState(_playlisttrack as PlaylistTrack);
}

class PlaylistTrackAddState extends State {
  PlaylistTrackAddState(this.playlisttrack);
  ...
  List<DropdownMenuItem<int>> _dropdownMenuItemsForPlaylistId =
      <DropdownMenuItem<int>>[];
  ...
  Widget build(BuildContext context) {
    void buildDropDownMenuForTrackId() async {
      ...
      setState(() {
        _dropdownMenuItemsForTrackId = dropdownMenuItems;
        _selectedTrackId = playlisttrack.TrackId;
      });
    }
    ...
    void buildDropDownMenuForPlaylistId() async {
      final dropdownMenuItems =
          await Playlist().select().toDropDownMenuInt('Name');
      setState(() {
        _dropdownMenuItemsForPlaylistId = dropdownMenuItems;
        _selectedPlaylistId = playlisttrack.PlaylistId;
      });
    }

    if (_dropdownMenuItemsForPlaylistId == null ||
        _dropdownMenuItemsForPlaylistId.isEmpty) {
      buildDropDownMenuForPlaylistId();
    }
    void onChangeDropdownItemForPlaylistId(int selectedPlaylistId) {
      setState(() {
    ...
    return Scaffold(
      appBar: AppBar(
        title: (playlisttrack.TrackId == null)
            ? Text('Add a new playlisttrack')
            : Text('Edit playlisttrack'),
      ),
      body: Container(
        alignment: Alignment.topCenter,
        width: double.infinity,
          ...
          child: Padding(
              ...
              child: Form(
                ...
                child: Column(
                  children: <Widget>[
                    buildRowTrackId(onChangeDropdownItemForTrackId),
                    buildRowPlaylistId(onChangeDropdownItemForPlaylistId),
                    FlatButton(
                      child: saveButton(),
                      onPressed: () {
                        if (_formKey.currentState.validate()) {
  ...
  Widget buildRowPlaylistId(
      void Function(int selectedPlaylistId) onChangeDropdownItemForPlaylistId) {
    return Row(
      children: <Widget>[
        Expanded(
          flex: 1,
        ...
        Expanded(
            ...
            child: Container(
              child: DropdownButtonFormField(
                ...
                items: _dropdownMenuItemsForPlaylistId,
                onChanged: onChangeDropdownItemForPlaylistId,
                validator: (value) {
                  if ((_selectedPlaylistId != null &&
                          _selectedPlaylistId.toString() != '0') ||
                      true) {
                    return null;
                  } else if (value == null || value == 0) {
                    return 'Please enter Playlist';
                  }
                  return null;
                },
              ),
  ...
  void save() async {
    ...
    final result = await playlisttrack.save();
    if (result != 0) {
      Navigator.pop(context, true);
    } else {
      UITools(context).alertDialog(playlisttrack.saveResult.toString(),
          title: 'save PlaylistTrack Failed!', callBack: () {
        Navigator.pop(context, true);
      });
    }
  }
