...
// RELATIONSHIPS
  ...
  if (isDeleted != null) {map["isDeleted"] = forQuery? (isDeleted ? 1 : 0):isDeleted;}
        ...
        Future<List<BoolResult>> saveAll(List<Product> products) async {
          final results = _mnProduct.saveAll("INSERT OR REPLACE INTO product (id, name,description,price,isActive,categoryId,rownum,isDeleted)  VALUES (?,?,?,?,?,?,?,?)",products);
          return results;
        }
        ...
        Future<int> _upsert() async {
          id = await _mnProduct.rawInsert(
              "INSERT OR REPLACE INTO product (id, name,description,price,isActive,categoryId,rownum,isDeleted)  VALUES (?,?,?,?,?,?,?,?)", [id,name,description,price,isActive,categoryId,rownum,isDeleted]);
          return id;
        }
        ...
        Future<List<BoolResult>> upsertAll(List<Product> products) async {
          final results = await _mnProduct.rawInsertAll(
              "INSERT OR REPLACE INTO product (id, name,description,price,isActive,categoryId,rownum,isDeleted)  VALUES (?,?,?,?,?,?,?,?)", products);
          return results;
        }
...
// COLLECTIONS
       Future<List<Product>> getProducts([VoidCallback productList(List<Product> o)]) async{
  ...
  if (isDeleted != null) {map["isDeleted"] = forQuery? (isDeleted ? 1 : 0):isDeleted;}
        ...
        Future<BoolResult> delete() async {
          ...
          var result= BoolResult();  result = await Product().select().categoryId.equals(id).delete();
  ...
  /// <returns>BoolResult res.success=Recovered, not res.success=Can not recovered</returns>
  Future<BoolResult> recover() async {
  print("SQFENTITIY: recover Category invoked (id=$id)");var result= BoolResult();  result = await Product().select(getIsDeleted: true).isDeleted.equals(true).and.categoryId.equals(id).update({"isDeleted": 0});
  if (!result.success) {return result;}
...
// endregion CategoryField
   ...
   class CategoryFilterBuilder extends SearchCriteria {
    ...
    Future<BoolResult> delete() async {
      _buildParameters();
      var r= BoolResult();    final idList = await toListPrimaryKey(null, false);
  ...
  _getIsDeleted = true;
  _buildParameters();
  print("SQFENTITIY: recover Category batch invoked");      final idList = await toListPrimaryKey(null, false);
      await Product().select(getIsDeleted: true).isDeleted.equals(true).and.categoryId.inValues(idList).update({"isDeleted": 0}); 
