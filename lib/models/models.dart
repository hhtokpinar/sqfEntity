...
class IdentitySequence {
  ...
  Future<int> reset([VoidCallback currentval(int o)]) async {
    final val = await SequenceManager().sequence(SequenceIdentity(), false, reset: true);
    if (currentval != null) {
      currentval(val);
    }
    return val;
...
// RELATIONSHIPS
  ...
  Future<int> save() async {
    if (id == null || id == 0) {
      id = await _mnProduct.insert(Product.withFields(name, description, price,
          isActive, categoryId, rownum, imageUrl, isDeleted));
    } else {
      id = await _upsert();
    }
    return id;
...
/*
      These classes was generated by SqfEntity
      To use these SqfEntity classes do following: 
      - import Todos2.dart into where to use
      - start typing Todos2().select()... (add a few filters with fluent methods)...(add orderBy/orderBydesc if you want)...
      - and then just put end of filters / or end of only select()  toSingle(todos2) / or toList(todos2List) 
      - you can select one Todos2 or List<Todos2> by your filters and orders
      - also you can batch update or batch delete by using delete/update methods instead of tosingle/tolist methods
        Enjoy.. Huseyin Tokpunar
      */
// region Todos2
class Todos2 {
  Todos2({this.id, this.userId, this.title, this.completed}) {
    setDefaultValues();
  }
  Todos2.withFields(this.userId, this.title, this.completed) {
    setDefaultValues();
  }
  Todos2.withId(this.id, this.userId, this.title, this.completed) {
    setDefaultValues();
  }
  Todos2.fromMap(Map<String, dynamic> o) {
    id = o["id"] as String;
    userId = o["userId"] as int;
    title = o["title"] as String;
    completed = o["completed"] != null ? o["completed"] == 1 : null;
  ...
  static const bool _softDeleteActivated = false;
  Todos2Manager __mnTodos2;
  Todos2FilterBuilder _select;

  Todos2Manager get _mnTodos2 {
    if (__mnTodos2 == null) __mnTodos2 = Todos2Manager();
    return __mnTodos2;
  }
  ...
  static Future<List<Todos2>> fromWeb(
      [VoidCallback todos2List(List<Todos2> o)]) async {
    final objList =
        await fromWebUrl("https://jsonplaceholder.typicode.com/todos");
    if (todos2List != null) {
      todos2List(objList);
  ...
  static Future<List<Todos2>> fromWebUrl(String url,
      [VoidCallback todos2List(List<Todos2> o)]) async {
    var objList = List<Todos2>();
    try {
      final response = await http.get(url);
      final Iterable list = json.decode(response.body) as Iterable;
      objList = list
          .map((todos2) => Todos2.fromMap(todos2 as Map<String, dynamic>))
          .toList();
      if (todos2List != null) {
        todos2List(objList);
      }
      ...
      print("SQFENTITY ERROR Todos2.fromWeb: ErrorMessage:" + e.toString());
      return null;
    }
  }

  static Future<List<Todos2>> fromObjectList(Future<List<dynamic>> o) async {
    final todos2sList = List<Todos2>();
    final data = await o;
    for (int i = 0; i < data.length; i++) {
      todos2sList.add(Todos2.fromMap(data[i] as Map<String, dynamic>));
    }
    return todos2sList;
  }

  static List<Todos2> fromMapList(List<Map<String, dynamic>> query) {
    final List<Todos2> todos2s = List<Todos2>();
    for (Map map in query) {
      todos2s.add(Todos2.fromMap(map as Map<String, dynamic>));
    }
    return todos2s;
  }

  /// returns Todos2 by ID if exist, otherwise returns null
  /// <param name="id">Primary Key Value</param>
  /// <returns>returns Todos2 if exist, otherwise returns null</returns>
  Future<Todos2> getById(String id) async {
    Todos2 todos2Obj;
    final data = await _mnTodos2.getById(id);
    if (data.length != 0) {
      todos2Obj = Todos2.fromMap(data[0] as Map<String, dynamic>);
    } else {
      todos2Obj = null;
    }
    return todos2Obj;
  ...
  /// <summary>
  /// Saves the object. If the Primary Key (id) field is null, returns Error.
  /// INSERTS (If not exist) OR REPLACES (If exist) data while Primary Key is not null.
  /// Call the saveAs() method if you do not want to save it when there is another row with the same id
  /// </summary>
  /// <returns>Returns BoolResult</returns>
  Future<BoolResult> save() async {
    final result = BoolResult(success: false);
    try {
      await _mnTodos2.rawInsert(
          "INSERT OR REPLACE INTO todos2 (id, userId,title,completed)  VALUES (?,?,?,?)",
          [id, userId, title, completed]);
      result.success = true;
    } catch (e) {
  ...
  /// <summary>
  /// saveAs Todos2
  /// Use this method if you do not want to update existing row when conflicts another row that have the same id
  /// </summary>
  ...
  Future<BoolResult> saveAs() async {
    ...
    try {
      await _mnTodos2.rawInsert(
          "INSERT INTO todos2 (id, userId,title,completed)  VALUES (?,?,?,?)",
          [id, userId, title, completed]);
      result.success = true;
    } catch (e) {
      result.errorMessage = e.toString();
  ...
  /// <summary>
  /// saveAll method saves the sent List<Todos2> as a batch in one transaction
  /// </summary>
  /// <returns> Returns a <List<BoolResult>> </returns>
  Future<List<BoolResult>> saveAll(List<Todos2> todos2s) async {
    final results = _mnTodos2.saveAll(
        "INSERT OR REPLACE INTO todos2 (id, userId,title,completed)  VALUES (?,?,?,?)",
        todos2s);
    return results;
  }
  ...
  /// </summary>
  /// <returns> Returns a <List<BoolResult>> </returns>
  Future<List<BoolResult>> upsertAll(List<Todos2> todos2s) async {
    final results = await _mnTodos2.rawInsertAll(
        "INSERT OR REPLACE INTO todos2 (id, userId,title,completed)  VALUES (?,?,?,?)",
        todos2s);
    return results;
  }
  ...
  /// <returns>BoolResult res.success=Deleted, not res.success=Can not deleted</returns>
  Future<BoolResult> delete([bool hardDelete = false]) async {
    print("SQFENTITIY: delete Todos2 invoked (id=$id)");
    if (!_softDeleteActivated || hardDelete) {
      return _mnTodos2.delete(QueryParams(whereString: "id=$id"));
    } else {
      return _mnTodos2
          .updateBatch(QueryParams(whereString: "id=$id"), {"isDeleted": 1});
    }
  ...
  //private Todos2FilterBuilder _Select;
  Todos2FilterBuilder select(
      {List<String> columnsToSelect, bool getIsDeleted}) {
    _select = Todos2FilterBuilder(this);
    _select._getIsDeleted = getIsDeleted == true;
    _select.qparams.selectColumns = columnsToSelect;
    return _select;
  }

  Todos2FilterBuilder distinct(
      {List<String> columnsToSelect, bool getIsDeleted}) {
    final Todos2FilterBuilder _distinct = Todos2FilterBuilder(this);
    _distinct._getIsDeleted = getIsDeleted == true;
    _distinct.qparams.selectColumns = columnsToSelect;
    _distinct.qparams.distinct = true;
    return _distinct;
...
// region Todos2Field
class Todos2Field extends SearchCriteria {
  Todos2Field(this.todos2FB) {
    param = DbParameter();
  }
  DbParameter param;
  String _waitingNot = "";
  Todos2FilterBuilder todos2FB;
  ...
  Todos2FilterBuilder equals(var pValue) {
    param.expression = "=";
    todos2FB._addedBlocks = _waitingNot == ""
        ? setCriteria(pValue, todos2FB.parameters, param, SqlSyntax.EQuals,
            todos2FB._addedBlocks)
        : setCriteria(pValue, todos2FB.parameters, param, SqlSyntax.NotEQuals,
            todos2FB._addedBlocks);
    _waitingNot = "";
    todos2FB._addedBlocks.needEndBlock[todos2FB._blockIndex] =
        todos2FB._addedBlocks.retVal;
    return todos2FB;
  }

  Todos2FilterBuilder isNull() {
    todos2FB._addedBlocks = setCriteria(
        0,
        todos2FB.parameters,
        param,
        SqlSyntax.IsNULL.replaceAll(SqlSyntax.NOT_KEYWORD, _waitingNot),
  ...
  Todos2FilterBuilder contains(dynamic pValue) {
    if (pValue != null) {
      todos2FB._addedBlocks = setCriteria(
          "%" + pValue.toString() + "%",
          todos2FB.parameters,
  ...
  Todos2FilterBuilder startsWith(dynamic pValue) {
    if (pValue != null) {
      todos2FB._addedBlocks = setCriteria(
          pValue.toString() + "%",
          todos2FB.parameters,
  ...
  Todos2FilterBuilder endsWith(dynamic pValue) {
    if (pValue != null) {
      todos2FB._addedBlocks = setCriteria(
          "%" + pValue.toString(),
          todos2FB.parameters,
  ...
  Todos2FilterBuilder between(dynamic pFirst, dynamic pLast) {
    if (pFirst != null && pLast != null) {
      todos2FB._addedBlocks = setCriteria(
          pFirst,
          todos2FB.parameters,
      ...
      if (_waitingNot != "") {
        todos2FB._addedBlocks = setCriteria(pFirst, todos2FB.parameters, param,
            SqlSyntax.LessThan, todos2FB._addedBlocks);
      } else {
        todos2FB._addedBlocks = setCriteria(pFirst, todos2FB.parameters, param,
            SqlSyntax.GreaterThanOrEquals, todos2FB._addedBlocks);
      }
    } else if (pLast != null) {
      if (_waitingNot != "") {
        todos2FB._addedBlocks = setCriteria(pLast, todos2FB.parameters, param,
            SqlSyntax.GreaterThan, todos2FB._addedBlocks);
      } else {
        todos2FB._addedBlocks = setCriteria(pLast, todos2FB.parameters, param,
            SqlSyntax.LessThanOrEquals, todos2FB._addedBlocks);
      }
    }
    _waitingNot = "";
    todos2FB._addedBlocks.needEndBlock[todos2FB._blockIndex] =
  ...
  Todos2FilterBuilder greaterThan(dynamic pValue) {
    param.expression = ">";
    todos2FB._addedBlocks = _waitingNot == ""
        ? setCriteria(pValue, todos2FB.parameters, param, SqlSyntax.GreaterThan,
            todos2FB._addedBlocks)
        : setCriteria(pValue, todos2FB.parameters, param,
            SqlSyntax.LessThanOrEquals, todos2FB._addedBlocks);
    _waitingNot = "";
    todos2FB._addedBlocks.needEndBlock[todos2FB._blockIndex] =
        todos2FB._addedBlocks.retVal;
  ...
  Todos2FilterBuilder lessThan(dynamic pValue) {
    param.expression = "<";
    todos2FB._addedBlocks = _waitingNot == ""
        ? setCriteria(pValue, todos2FB.parameters, param, SqlSyntax.LessThan,
            todos2FB._addedBlocks)
        : setCriteria(pValue, todos2FB.parameters, param,
            SqlSyntax.GreaterThanOrEquals, todos2FB._addedBlocks);
    _waitingNot = "";
  ...
  Todos2FilterBuilder greaterThanOrEquals(dynamic pValue) {
    param.expression = ">=";
    todos2FB._addedBlocks = _waitingNot == ""
        ? setCriteria(pValue, todos2FB.parameters, param,
            SqlSyntax.GreaterThanOrEquals, todos2FB._addedBlocks)
  ...
  Todos2FilterBuilder lessThanOrEquals(dynamic pValue) {
    param.expression = "<=";
    todos2FB._addedBlocks = _waitingNot == ""
        ? setCriteria(pValue, todos2FB.parameters, param,
            SqlSyntax.LessThanOrEquals, todos2FB._addedBlocks)
  ...
  Todos2FilterBuilder inValues(var pValue) {
    todos2FB._addedBlocks = setCriteria(
        pValue,
        todos2FB.parameters,
        param,
...
// endregion Todos2Field

// region Todos2FilterBuilder
class Todos2FilterBuilder extends SearchCriteria {
  Todos2FilterBuilder(Todos2 obj) {
    whereString = "";
    qparams = QueryParams();
    parameters = List<DbParameter>();
    orderByList = List<String>();
  ...
  Todos2FilterBuilder get startBlock {
    _addedBlocks.waitingStartBlock.add(true);
    _addedBlocks.needEndBlock.add(false);
    _blockIndex++;
    if (_blockIndex > 1) _addedBlocks.needEndBlock[_blockIndex - 1] = true;
  ...
  Todos2FilterBuilder where(String whereCriteria) {
    if (whereCriteria != null && whereCriteria != "") {
      final DbParameter param = DbParameter();
      _addedBlocks = setCriteria(
          0, parameters, param, "(" + whereCriteria + ")", _addedBlocks);
  ...
  Todos2FilterBuilder page(int page, int pagesize) {
    if (page > 0) _page = page;
    if (pagesize > 0) _pagesize = pagesize;
    return this;
  }

  Todos2FilterBuilder top(int count) {
    if (count > 0) {
      _pagesize = count;
    }
    return this;
  ...
  Todos2FilterBuilder get endBlock {
    if (_addedBlocks.needEndBlock[_blockIndex]) {
      parameters[parameters.length - 1].whereString += " ) ";
    }
    _addedBlocks.needEndBlock.removeAt(_blockIndex);
  ...
  Todos2FilterBuilder orderBy(var argFields) {
    if (argFields != null) {
      if (argFields is String) {
        orderByList.add(argFields);
      } else {
  ...
  Todos2FilterBuilder orderByDesc(var argFields) {
    if (argFields != null) {
      if (argFields is String) {
        orderByList.add("$argFields desc ");
      } else {
  ...
  Todos2FilterBuilder groupBy(var argFields) {
    if (argFields != null) {
      if (argFields is String) {
        groupByList.add(" $argFields ");
      } else {
  ...
  Todos2Field setField(Todos2Field field, String colName, DbType dbtype) {
    field = Todos2Field(this);
    field.param = DbParameter(
        dbType: dbtype,
        columnName: colName,
        wStartBlock: _addedBlocks.waitingStartBlock[_blockIndex]);
  ...
  Todos2Field _userId;
  Todos2Field get userId {
    _userId = setField(_userId, "userId", DbType.integer);
    return _userId;
  }

  Todos2Field _title;
  Todos2Field get title {
    _title = setField(_title, "title", DbType.text);
    return _title;
  }

  Todos2Field _completed;
  Todos2Field get completed {
    _completed = setField(_completed, "completed", DbType.bool);
    return _completed;
  }
  ...
  void _buildParameters() {
    ...
    if (Todos2._softDeleteActivated) {
      if (whereString != "") {
        whereString = (!_getIsDeleted ? "ifnull(isDeleted,0)=0 AND" : "") +
            " ($whereString)";
      } else if (!_getIsDeleted) {
  ...
  /// <summary>
  /// Deletes List<Todos2> batch by query
  /// </summary>
  /// <returns>BoolResult res.success=Deleted, not res.success=Can not deleted</returns>
  Future<BoolResult> delete([bool hardDelete = false]) async {
    _buildParameters();
    var r = BoolResult();
    if (Todos2._softDeleteActivated && !hardDelete) {
      r = await _obj._mnTodos2.updateBatch(qparams, {"isDeleted": 1});
    } else {
      r = await _obj._mnTodos2.delete(qparams);
    }
    return r;
  }
  ...
  Future<BoolResult> update(Map<String, dynamic> values) {
    _buildParameters();
    return _obj._mnTodos2.updateBatch(qparams, values);
  }

  /// This method always returns Todos2Obj if exist, otherwise returns null
  /// <returns>List<Todos2></returns>
  Future<Todos2> toSingle([VoidCallback todos2(Todos2 o)]) async {
    _pagesize = 1;
    _buildParameters();
    final objFuture = _obj._mnTodos2.toList(qparams);
    final data = await objFuture;
    Todos2 retVal;
    if (data.isNotEmpty) {
      retVal = Todos2.fromMap(data[0] as Map<String, dynamic>);
    } else {
      retVal = null;
    }
    if (todos2 != null) {
  ...
  /// This method always returns int.
  /// <returns>int</returns>
  Future<int> toCount([VoidCallback todos2Count(int c)]) async {
    _buildParameters();
    qparams.selectColumns = ["COUNT(1) AS CNT"];
    final todos2sFuture = await _obj._mnTodos2.toList(qparams);
    final int count = todos2sFuture[0]["CNT"] as int;
    if (todos2Count != null) {
      todos2Count(count);
    }
    return count;
  ...
  /// This method always returns List<Todos2>.
  /// <returns>List<Todos2></returns>
  Future<List<Todos2>> toList([VoidCallback todos2List(List<Todos2> o)]) async {
    _buildParameters();
    final todos2sFuture = _obj._mnTodos2.toList(qparams);
    final List<Todos2> todos2sData = List<Todos2>();
    final data = await todos2sFuture;
    final int count = data.length;
    for (int i = 0; i < count; i++) {
      todos2sData.add(Todos2.fromMap(data[i] as Map<String, dynamic>));
    }
    if (todos2List != null) todos2List(todos2sData);
    return todos2sData;
  }

  /// Returns List<DropdownMenuItem<Todos2>>
  Future<List<DropdownMenuItem<Todos2>>> toDropDownMenu(
      String displayTextColumn,
      [VoidCallback dropDownMenu(List<DropdownMenuItem<Todos2>> o)]) async {
    _buildParameters();
    final todos2sFuture = _obj._mnTodos2.toList(qparams);

    final data = await todos2sFuture;
    final int count = data.length;
    final List<DropdownMenuItem<Todos2>> items = List();
    items.add(DropdownMenuItem(
      value: Todos2(),
      child: Text("Select Todos2"),
    ));
    for (int i = 0; i < count; i++) {
      items.add(
        DropdownMenuItem(
          value: Todos2.fromMap(data[i] as Map<String, dynamic>),
          child: Text(data[i][displayTextColumn].toString()),
        ),
      );
    }
  ...
  Future<List<int>> toListPrimaryKey(
    ...
    final List<int> idData = List<int>();
    qparams.selectColumns = ["id"];
    final idFuture = await _obj._mnTodos2.toList(qparams);
  ...
  Future<List<dynamic>> toListObject(
    ...
    final objectFuture = _obj._mnTodos2.toList(qparams);
  ...
  /// Returns List<String> for selected first column
  /// Sample usage: await Todos2.select(columnsToSelect: ["columnName"]).toListString()
  Future<List<String>> toListString(
      [VoidCallback listString(List<String> o)]) async {
    _buildParameters();
...
// endregion Todos2Fields

//region Todos2Manager
class Todos2Manager extends SqfEntityProvider {
  Todos2Manager() : super(MyDbModel(), tableName: _tableName, colId: _colId);
  static String _tableName = "todos2";
  static String _colId = "id";
}
//endregion Todos2Manager
