...
// RELATIONSHIPS
  ...
  // END RELATIONSHIPS
      ...
      Product({this.id, this.name,this.description,this.price,this.isActive,this.categoryId,this.rownum,this.isDeleted}) { setDefaultValues();}
      Product.withFields(this.name,this.description,this.price,this.isActive,this.categoryId,this.rownum,this.isDeleted){ setDefaultValues();}
      Product.withId(this.id, this.name,this.description,this.price,this.isActive,this.categoryId,this.rownum,this.isDeleted){ setDefaultValues();}
  ...
  this.isDeleted = o["isDeleted"] != null ? o["isDeleted"] == 1 : null;
      ...
      List<dynamic> toArgs() {
        return[id,name,description,price,isActive,categoryId,rownum,isDeleted];   
      }  
  
        
      static fromWebUrl(String url, VoidCallback  productList (List<Product> o)) async {
      ...
      Future<int> upsert() async {
        id = await _mnProduct.rawInsert(
            "INSERT OR REPLACE INTO [\$table] (id, name,description,price,isActive,categoryId,rownum,isDeleted)  VALUES (?,?,?,?,?,?,?,?)", [id,name,description,price,isActive,categoryId,rownum,isDeleted]);
        return id;
      }
      ...
      Future<List<BoolResult>> upsertAll(List<Product> products) async {
        var results = await _mnProduct.rawInsertAll(
            "INSERT OR REPLACE INTO [\$table] (id, name,description,price,isActive,categoryId,rownum,isDeleted)  VALUES (?,?,?,?,?,?,?,?)", products);
        return results;
      }
...
// COLLECTIONS
  ...
  this.isDeleted = o["isDeleted"] != null ? o["isDeleted"] == 1 : null;
      ...
      static fromWebUrl(String url, VoidCallback  categoryList (List<Category> o)) async {
      ...
      Future<List<Category>> fromObjectList(Future<List<dynamic>> o) async {
      ...
      List<Category> fromMapList(List<Map<String, dynamic>> query) {
      ...
      Future<int> save() async {
        if (id == null || id == 0)
          id = await _mnCategory.insert(
              Category.withFields(name,isActive,isDeleted));
        else
          _mnCategory.update(
              Category.withId(id, name,isActive,isDeleted));
        return id;
      }
      ...
      Future<int> upsert() async {
        id = await _mnCategory.rawInsert(
            "INSERT OR REPLACE INTO [\$table] (id, name,isActive,isDeleted)  VALUES (?,?,?,?)", [id,name,isActive,isDeleted]);
        return id;
      }
      ...
      Future<List<BoolResult>> upsertAll(List<Category> categories) async {
        var results = await _mnCategory.rawInsertAll(
            "INSERT OR REPLACE INTO [\$table] (id, name,isActive,isDeleted)  VALUES (?,?,?,?)", categories);
        return results;
      }
      ...
      /// <returns>true=Deleted, false=Can not deleted</returns>
      Future<BoolResult> delete() async {
        print("SQFENTITIY: delete Category called (id=$id)");
        if (!softDeleteActivated)
...
class CategoryFilterBuilder extends SearchCriteria {
  ...
  void _buildParameters() {
    ...
    if (Category.softDeleteActivated) {
      if (whereString != "")
        whereString = (!_getIsDeleted ? "ifnull(isDeleted,0)=0 AND" : "") +
            " ($whereString)";
      else if (!_getIsDeleted) whereString = "ifnull(isDeleted,0)=0";
  ...
  void toList(VoidCallback categoryList (List<Category> o)) async {
    ...
    var categorysFuture = _obj._mnCategory.toList(qparams);

    List<Category> categorysData = new List<Category>();
    categorysFuture.then((data) {
      int count = data.length;
      for (int i = 0; i < count; i++) {
        categorysData.add(Category.fromMap(data[i]));
      }
      categoryList (categorysData);
      categorysData = null;
    });
...
// end FIELDS
      ...
      static const bool softDeleteActivated=false;
    ...
    this.completed = o["completed"] != null ? o["completed"] == 1 : null;
      ...
      Future<List<Todo>> fromObjectList(Future<List<dynamic>> o) async {
      ...
      List<Todo> fromMapList(List<Map<String, dynamic>> query) {
      ...
      Future<int> save() async {
        if (id == null || id == 0)
          id = await _mnTodo.insert(
              Todo.withFields(userId,title,completed));
        else
          _mnTodo.update(
              Todo.withId(id, userId,title,completed));
        return id;
      }
      ...
      Future<List<BoolResult>> saveAll(List<Todo> todos) async {
        var results = _mnTodo.saveAll(todos);
        return results;
      }
      ...
      Future<int> upsert() async {
        id = await _mnTodo.rawInsert(
            "INSERT OR REPLACE INTO [\$table] (id, userId,title,completed)  VALUES (?,?,?,?)", [id,userId,title,completed]);
        return id;
      }
      ...
      Future<List<BoolResult>> upsertAll(List<Todo> todos) async {
        var results = await _mnTodo.rawInsertAll(
            "INSERT OR REPLACE INTO [\$table] (id, userId,title,completed)  VALUES (?,?,?,?)", todos);
        return results;
      }
      ...
      /// <returns>true=Deleted, false=Can not deleted</returns>
      Future<BoolResult> delete() async {
        print("SQFENTITIY: delete Todo called (id=$id)");
        if (!softDeleteActivated)
