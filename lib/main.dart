...
void main(List<String> args) {
  ...
  // If needed, initilizeDb method runs that CREATE / ALTER TABLE queries for you.
...
void printProducts(bool getIsDeleted) {
  Product().select(getIsDeleted: getIsDeleted).toList((productList) {
    print(
        "EXAMPLE 2.1: SELECT ALL ROWS WITHOUT FILTER ex: SELECT * FROM PRODUCTS \n -> Product().select().toList()");
...
void samples2() {
// EXAMPLE 2.1: SELECT * FROM PRODUCTS
  printProducts(false);

// EXAMPLE 2.2: ORDER BY FIELDS -> ex: SELECT * FROM PRODUCTS ORDER BY name, price DESC, id
  Product()
      .select()
      .orderBy("name")
      .orderByDesc("price")
...
// EXAMPLE 2.3: SELECT SPECIFIC FIELDS -> ex: SELECT name,price FROM PRODUCTS ORDER BY price DESC
  Product()
...
void samples3() {
// EXAMPLE 3.1: SELECT * FROM PRODUCTS WHERE isActive=1
  Product().select().isActive.equals(true).toList((productList) {
    print(
        "EXAMPLE 3.1: EQUALS ex: SELECT * FROM PRODUCTS WHERE isActive=1 \n->  Product().select().isActive.equals(true).toList()");
    print("${productList.length} matches found:");
    for (int i = 0; i < productList.length; i++) {
      print(productList[i].toMap());
    }
...
// EXAMPLE 3.2: SELECT * FROM PRODUCTS WHERE ID IN (3,6,9)
  Product().select().id.inValues([3, 6, 9]).toList((productList) {
    print(
        "EXAMPLE 3.2: WHERE field IN (VALUES) ex: SELECT * FROM PRODUCTS WHERE ID IN (3,6,9) \n -> Product().select().id.inValues([3,6,9]).toList()");
    print("${productList.length} matches found:");
    for (int i = 0; i < productList.length; i++) {
      print(productList[i].toMap());
    }
...
// EXAMPLE 3.3: SELECT TOP 1 * FROM PRODUCTS WHERE price>10000 AND (description LIKE '%256%' OR description LIKE '512%')
  Product()
      .select()
      .price
      .greaterThan(10000)
...
// EXAMPLE 3.4: SELECT name,price FROM PRODUCTS WHERE price <=10000 AND (description LIKE '%128%' OR description LIKE '%GB')
  Product()
      .select()
      .price
...
// EXAMPLE 3.6: GREATERTHEN OR EQUALS, LESSTHAN OR EQUALS
  Product()
      .select()
      .price
      ...
      .toList((productList) {
    print(
        "EXAMPLE 3.6: GREATERTHEN OR EQUALS, LESSTHAN OR EQUALS ex: SELECT * FROM PRODUCTS WHERE price>=10000 AND price<=13000 \n -> Product().select().price.greaterThanOrEquals(10000).and.price.lessThanOrEquals(13000).toList();");
    print("${productList.length} matches found:");
    for (int i = 0; i < productList.length; i++) {
      print(productList[i].toMap());
    }
...
// EXAMPLE 3.7: BETWEEN KEYWORD
  Product()
      ...
      .toList((productList) {
    print(
        "EXAMPLE 3.7: BETWEEN ex: SELECT * FROM PRODUCTS WHERE price BETWEEN 8000 AND 14000 \n -> Product().select().price.between(8000,14000).orderBy(\"price\").toList();");
    print("${productList.length} matches found:");
    for (int i = 0; i < productList.length; i++) {
      print(productList[i].toMap());
    }
...
// EXAMPLE 3.9: WRITING CUSTOM FILTER IN WHERE CLAUSE
  Product().select().where("id IN (3,6,9) OR price>8000").toList((productList) {
    print(
        "EXAMPLE 3.9: WRITING CUSTOM FILTER IN WHERE CLAUSE ex: SELECT * FROM PRODUCTS WHERE id IN (3,6,9) OR price>8000 \n -> Product().select().where(\"id IN (3,6,9) OR price>8000\").toList()");
    print("${productList.length} matches found:");
    for (int i = 0; i < productList.length; i++) {
      print(productList[i].toMap());
    }
  ...
  // EXAMPLE 3.10: Build filter and query from values from the form
  ...
  Product()
      ...
      .toList((productList) {
    print(
        "EXAMPLE 3.10: Product().select().price.between($minPrice, $maxPrice).and.name.contains(\"$nameContains\").and.description.contains(\"$descriptionContains\").toList()");
    print("${productList.length} matches found:");
  ...
  // EXAMPLE 3.11: Select products with deleted items (only softdelete was activated on Model)
  Product().select(getIsDeleted: true).toList((productList) {
    print(
        "EXAMPLE 3.11: EXAMPLE 1.13: Select products with deleted items\n -> Product().select(getIsDeleted: true).toList()");
    print("${productList.length} matches found:");
  ...
  // EXAMPLE 3.12: Select products only deleted items (only softdelete was activated on Model)
  Product()
...
void samples4() {
// EXAMPLE 4.1: SELECT TOP 3 * FROM PRODUCTS ORDER BY price DESC
  Product().select().orderByDesc("price").top(3).toList((productList) {
    print(
        "EXAMPLE 4.1: LIMITATION ex: SELECT TOP 3 * FROM PRODUCTS ORDER BY price DESC \n -> Product().select().orderByDesc(\"price\").top(3).toList()");
    print("${productList.length} matches found:");
    for (int i = 0; i < productList.length; i++) {
      print(productList[i].toMap());
    }
  ...
  // EXAMPLE 4.2: SAMPLE PAGING -> PRODUCTS in 3. page (5 items per page)
  Product().select().page(3, 5).toList((productList) {
    print(
        "EXAMPLE 4.2: SAMPLE PAGING ex: PRODUCTS in 3. page (5 items per page) \n -> Product().select().page(3,5).toList()");
    print("${productList.length} matches found:");
    for (int i = 0; i < productList.length; i++) {
      print(productList[i].toMap());
    }
...
void samples5() {
// EXAMPLE 5.1: SELECT DISTINCT name FROM PRODUCTS WHERE price > 3000
  Product()
      ...
      .toList((productList) {
        print(
            "EXAMPLE 5.1: DISTINCT ex: SELECT DISTINCT name FROM PRODUCTS WHERE price > 3000 \n -> Product().distinct(columnsToSelect:[\"name\").price.greaterThan(3000).toList();");
        print("${productList.length} matches found:");
        for (int i = 0; i < productList.length; i++) {
          print(productList[i].toMap());
        }
  ...
  // EXAMPLE 5.2: GROUP BY with SCALAR OR AGGREGATE FUNCTIONS ex: SELECT COUNT(id) AS Count, MIN(price) AS minPrice, MAX(price) AS maxPrice, AVG(price) AS avgPrice FROM PRODUCTS GROUP BY name
...
void samples6() {
// EXAMPLE 6.1: Update multiple records with query
...
// EXAMPLE 6.2: Update multiple records with query
  Product()
      .select()
      .id
      .lessThanOrEquals(10)
    ...
    print(
        "EXAMPLE 6.2: uUpdate multiple records with query \n -> Product().select().id.lessThanOrEquals(10).update({\"isActive\": 1});");
    print(result.toString());
    print(
        "---------------------------------------------------------------\n\n");
  });

// EXAMPLE 6.3: select product by id and update
...
// update product object if exist
    if (product2 != null) {
      ...
      print("EXAMPLE 6.3: id=15 => product not found");
    print(
        "---------------------------------------------------------------\n\n");
  });

// EXAMPLE 6.4: update some filtered products with saveAll method -> Product().saveAll(productList){});
  Product().select().toList((productList) {
    ...
    Product().saveAll(productList).then((results) {
      Product().select().toList((productList) {
        print(
            "EXAMPLE 6.4: update some filtered products with saveAll method \n -> Product().saveAll(productList){});");
        ...
        print(
            "---------------------------------------------------------------");
        print(
            "EXAMPLE 6.4: listing saved products (set rownum=i) with saveAll method;");
        for (int i = 0; i < productList.length; i++) {
          print(productList[i].toMap());
        }
        print(
...
void samples1(VoidCallback completed(bool c)) async {
  // EXAMPLE 1.1 : Delete all products.
  // Uncomment following section for delete all products
//  await sample1_1((c) {});

// EXAMPLE 1.2: get product with query id and delete
  sample1_2((_) {
// EXAMPLE 1.3: Get product by id and then delete
    sample1_3((_) {
// EXAMPLE 1.4: Delete many products by filter
      sample1_4((_) {
        completed(true);
      });
    });
...
sample1_1(VoidCallback completed(bool c)) {
  Product().select().delete().then((result) {
    print("EXAMPLE 1.1: Delete all products \n -> Product().select().delete()");
    if (result.success)
...
sample1_2(VoidCallback completed(bool c)) {
  Product().select().id.equals(16).delete().then((result) {
    print(
        "EXAMPLE 1.2: delete product by query filder \n -> Product().select().id.equals(16).delete();");
    print(result.toString());
    print(
        "---------------------------------------------------------------\n\n");
    completed(true);
...
sample1_3(VoidCallback completed(bool c)) {
...
// delete product object if exist
    if (product != null) {
      product.delete().then((result) {
        print(
            "EXAMPLE 1.3: delete product if exist \n -> if (product != null) Product.delete();");
        if (result.success)
          print("${result.successMessage}");
        else
          print("${result.errorMessage}");
...
sample1_4(VoidCallback completed(bool c)) {
